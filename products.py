...
def new_product(product_name, product_type, product_number, price, user_id):
    if user_id == 0:
        return False
    sql = text(
        "INSERT INTO products "
        "(product_name, type, product_number, price, user_id) "
        "VALUES (:product_name, :type, :product_number, :price, :user_id)"
    )
...
def product_register(user_id):
    sql = text(
        "SELECT P.id, P.product_name, P.type, P.product_number, P.price "
        "FROM products P "
...
def remove_product(product_id):
    try:
        sql = text("UPDATE products SET visible=FALSE WHERE id=:product_id")
