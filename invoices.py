...
def new_invoice(biller_id, customer_id, form_time, invoice_number, product_one_id,
                ...
                no_margin_sum, margin, total_sum, user_id):
    if user_id == 0:
        return False
    sql = text(
        "INSERT INTO invoices (biller_id, customer_id, form_time, invoice_number, "
        "product_one_id, product_two_id, product_three_id, product_four_id, product_five_id, "
        "no_margin_sum, margin, sum, user_id) VALUES (:biller_id, :customer_id, :form_time, "
        ":invoice_number, :product_one_id, :product_two_id, :product_three_id, :product_four_id, "
        ":product_five_id, :no_margin_sum, :margin, :sum, :user_id)"
    )
    db.session.execute(sql, {
        ...
        "product_four_id": product_four_id,
        "product_five_id": product_five_id,
...
def invoice_archive(user_id):
    sql = text(
        "SELECT invoice_number, form_time, biller_id, customer_id, "
        "product_one_id, product_two_id, product_three_id, product_four_id, "
        "product_five_id, no_margin_sum, margin, sum "
        "FROM invoices I "
        "WHERE I.user_id = :user_id AND I.visible = TRUE "
        "ORDER BY I.id"
    )
