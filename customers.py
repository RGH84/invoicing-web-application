...
def new_customer(customer_name, address, phonenumber, business_id, user_id):
    ...
    sql = text(
        ...
        "(customer_name, address, phonenumber, business_id, user_id) "
        "VALUES (:customer_name, :address, :phonenumber, :business_id, :user_id)"
    )
...
def customer_register(user_id):
    sql = text(
        "SELECT C.id, C.customer_name, C.address, C.phonenumber, C.business_id "
        "FROM customers C "
        "WHERE C.user_id = :user_id AND C.visible = TRUE "
        "ORDER BY C.id"
    )
